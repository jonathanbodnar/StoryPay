/**
 * Thin fetch wrapper that retries once on a gateway-level failure
 * (502/503/504) after a short delay.
 *
 ...
 * (never a validation or auth problem), so a single silent retry is safe:
 * on both the app-level and platform-level paths that produce a 502 here, no
 * partial send has occurred yet.
 *
