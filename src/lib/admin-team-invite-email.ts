...
import { buildSystemEmail } from './email-templates';
...
export async function sendAdminInviteEmail(
  ...
  const html = buildSystemEmail({
    title:   subject,
    heading: headline,
    bodyHtml: `<p style="margin:0 0 20px;font-size:15px;color:#4b5563;line-height:1.55;">${intro}</p>
      ...
      <p style="margin:20px 0 4px;font-size:13px;color:#6b7280;">Access level</p>
      <p style="margin:0;font-size:14px;color:#111827;">${escapeHtml(access)}</p>`,
    cta:        { label: 'Sign in to admin panel', url: loginUrl },
    footerHtml: `<p style="margin:0;font-size:12px;color:#9ca3af;line-height:1.55;text-align:center;">If you didn't expect this invite, you can ignore this email. Questions? Reply to this message.</p>`,
  });
