...
function InlineAI() {
 ...
 className="w-full text-left rounded-2xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-700 hover:border-gray-300 hover: transition-all">
 {p}
 </button>
 ))}
 </div>
 ...
 <div className="h-6 w-6 rounded-full flex items-center justify-center"style={{ backgroundColor: '#525252' }}>
 <Sparkles size={11} className="text-white"/>
 </div>
...
export default function HelpPage() {
 ...
 className={`w-full rounded-2xl border bg-white pl-10 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-100 transition-colors ${isListening ? 'border-red-300 ring-2 ring-red-100' : 'border-gray-200'} ${(query || speechSupported) ? 'pr-16' : 'pr-4'}`}
 />
 ...
 className="w-full text-left rounded-2xl border border-gray-200 bg-white hover: hover:border-gray-300 transition-all p-4"
 >
 ...
 className="w-full flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-left hover:bg-gray-100 hover:border-gray-200 transition-colors group"
 >
 ...
 className="w-full text-left rounded-2xl border border-gray-200 bg-white hover: hover:border-gray-300 transition-all px-5 py-4 flex items-center justify-between gap-3"
 >
 <div>
 ...
 className="w-full flex items-start justify-between gap-3 px-4 py-3.5 text-left">
                                            <div className="flex min-w-0 flex-1 items-start gap-3">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <p className="text-xs text-gray-400 leading-tight mt-1 tabular-nums">{cat.articles.length} article{cat.articles.length !== 1 ? 's' : ''}</p>
                                                </div>
                                            </div>
 ...
 <ChevronRight size={13} className="text-gray-300 flex-shrink-0"/>
                                ...
                                <div className="hidden lg:grid grid-cols-2 xl:grid-cols-3 gap-4 auto-rows-fr">
                                    {CATEGORIES.map(cat => {
                                        const Icon = cat.icon;
                                        return (
                                            <button
                                                ...
                                                className="h-full flex flex-col text-left rounded-2xl border border-gray-200 bg-white hover: hover:border-gray-300 transition-all p-5 group"
                                            >
                                                <div className="flex items-start gap-3 mb-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <p className="text-sm font-bold leading-tight text-gray-900 group-hover:text-gray-700">{cat.label}</p>
                                                        <p className="text-xs text-gray-400 leading-tight mt-1">{cat.articles.length} article{cat.articles.length !== 1 ? 's' : ''}</p>
                                                    </div>
                                                </div>
                                                <div className="space-y-1.5 flex-1">
                                                    {cat.articles.slice(0, 3).map(a => (
                                                        <p key={a.id} className="text-xs text-gray-500 flex items-start gap-1.5">
