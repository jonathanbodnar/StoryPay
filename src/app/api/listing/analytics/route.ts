...
import { normalizeGa4MeasurementId } from '@/lib/ga4';
...
export async function GET() {
  ...
  const { data, error } = await supabaseAdmin
    .from('venues')
    .select('ga4_measurement_id, slug, name')
    .eq('id', venueId)
    .maybeSingle();
  ...
  const row = data as { ga4_measurement_id: string | null; slug: string | null; name: string | null };
 ...
 * body: { ga4_measurement_id: string | null } — empty string clears; invalid format returns 400.
 */
export async function PATCH(request: NextRequest) {
  const venueId = await getVenueId();
  if (!venueId) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

  let body: { ga4_measurement_id?: string | null };
  try {
  ...
  if (!('ga4_measurement_id' in body)) {
    return NextResponse.json({ error: 'ga4_measurement_id is required' }, { status: 400 });
  }
  ...
  const { data, error } = await supabaseAdmin
    .from('venues')
    .update({ ga4_measurement_id: next })
    .eq('id', venueId)
    .select('ga4_measurement_id, slug, name')
    .maybeSingle();
