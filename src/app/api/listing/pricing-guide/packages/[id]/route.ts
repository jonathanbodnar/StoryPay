...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const owns = await childBelongsToVenue('venue_pricing_guide_packages', id, venueId);
  ...
  for (const [k, v] of Object.entries(body)) {
    ...
    if (k === 'included_items' && Array.isArray(v)) {
      update[k] = (v as unknown[]).filter((s) => typeof s === 'string');
    } else {
      update[k] = v;
    }
  }
