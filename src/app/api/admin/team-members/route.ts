...
export async function POST(request: NextRequest) {
  ...
  // Treat existing-email as a re-invite: refresh credentials + permissions
  // and resend the welcome email. This way the inviter can keep clicking
  // "Invite team member" to retry without manually deleting the row first.
  ...
  // Errors here don't fail the API response — the member row is saved either
  // way, and the inviter can always click "Resend invite" to retry the email.
  let emailSent = false;
  ...
  try {
    ...
    console.error('[team-members] invite email exception:', emailError);
  }

  return NextResponse.json(
    { member: row, emailSent, emailError, isReinvite },
    { status: isReinvite ? 200 : 201 },
  );
}
