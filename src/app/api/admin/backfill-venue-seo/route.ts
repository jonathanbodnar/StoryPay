/**
 * POST /api/admin/backfill-venue-seo
 *
 * Retroactively generates SEO metadata for every published venue that
 * doesn't have it yet (or all of them with ?force=1). Runs sequentially to
 * be gentle on the LLM API; processes up to `limit` venues per call
 * (default 25) and reports how many remain, so you can call it repeatedly
 * until remaining = 0.
 *
...
import { generateVenueSeo } from '@/lib/venue-seo';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  for (const id of batch) {
    const res = await generateVenueSeo(id);
    if (res.ok) generated++;
    else errors.push(`${id}: ${res.error}`);
    // Missing columns means migration 173 hasn't run — stop immediately.
    if (res.error === 'seo_columns_missing') break;
  }
