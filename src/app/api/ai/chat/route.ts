...
- Venue listing (sidebar flyout, Store icon): **Bride Booking System™ Analytics** (/dashboard/listing) — the primary analytics hub showing the live visitor map, booking funnel (Leads → Conversations → Tours → Weddings), KPI cards, daily views chart, traffic sources, geography, lead insights, UTM builder, and QR code. **Free-plan users see this page blurred with an upgrade overlay.** **Venue Listing Editor** (/dashboard/listing/venue-listing) — edit how the venue appears on storyvenue.com (description, slug, capacity, publish toggle). **Photos** — cover + gallery for the directory listing. **Reviews** — StoryVenue reviews + Google reviews. **Speed to Lead System** — 5-phase automation. **Lead Link™** — a shareable link-in-bio landing page for social profiles (/dashboard/listing/lead-link). Paths: /dashboard/listing (analytics), /dashboard/listing/venue-listing (editor), /dashboard/listing/media, /dashboard/listing/images, /dashboard/listing/analytics, /dashboard/listing/reviews, /dashboard/listing/booking-system, /dashboard/listing/lead-link.
- Wedding Hub (top-level sidebar, heart icon): a shared portal where the venue and each booked couple plan the wedding together — guest list & RSVPs, seating chart, meal selections, and shared messaging in one place. Path: /dashboard/wedding-hub. The couple has a matching "Wedding Hub" inside their own login, so both sides use the same name. Included on private-client (Legacy / All-Inclusive) plans; an add-on on $97 / Free plans.
...
## Wedding Hub (shared portal for you and your booked couples)
- Tagline: "Wedding Hub — one shared place for you and your venue." Path: sidebar → **Wedding Hub** (heart icon, top-level, /dashboard/wedding-hub). The same "Wedding Hub" name appears in the couple's own login, so both sides know exactly where to go.
- Connecting a couple works both ways: send an invite from your Wedding Hub (enter the couple's email and they get a link to connect), OR a couple finds your venue and requests to connect. Either way the venue approves the link, and one couple links to one venue.
...
- Seating chart: the couple creates reception tables (name + seats) and seats each guest party; you see the table layout with seated/capacity counts for day-of room setup.
- Shared messaging: you and the couple message in one thread, so wedding-day details don't scatter across email and texts.
- You control what's shared: on the Wedding Hub page, toggle which wedding details the couple sees (wedding date, guest count, space/room, coordinator).
