...
- Connecting a couple works both ways: send an invite from your Wedding Planner (enter the couple's email and they get a link to connect), OR a couple finds your venue and requests to connect. Either way the venue approves the link, and one couple links to one venue.
- Guest list & RSVPs: the couple builds and owns their guest list (names, party size, meal choice, dietary notes, groups) and can email self-service RSVP links to their guests. You get a live rollup — total invited, attending/declined/awaiting, headcount, and meal tallies for your BEO — without seeing guest contact details, which stay with the couple.
- Seating chart & room layout: the couple creates reception tables (name + number of seats) and seats each guest party in their seating list; seats fill by party size, with an over-capacity warning when a table is oversubscribed. Tables are then placed on a shared drag-and-drop floor plan that BOTH you and the couple edit — one plan, so a change either side makes is what the other sees. The canvas opens in View mode: tap or click any table and a panel shows exactly who is seated there, grouped by party, with party size, RSVP status, meal choice and group. On a computer, hovering a table shows a quick preview of the names. Edit mode is where items are moved, resized, rotated or deleted, and where tables and decor are added. Decor includes dance floor, head table, bar, DJ, gift table, cake, stage, text label, and rows of chairs you can size from 1 to 10 (useful for ceremony seating). Tables come in round, square and long shapes. Print or download the plan as a PNG — with "Include guest names" on, each table is labelled with its name, seated count and the parties seated there, giving your team a named seating chart for day-of room setup. Unsaved layout edits are protected: leaving the canvas, collapsing the couple, or switching away prompts before discarding changes.
- Shared messaging: you and the couple message in one thread, so wedding-day details don't scatter across email and texts.
- You control what's shared: on the Wedding Planner page, toggle which wedding details the couple sees (wedding date, guest count, space/room, coordinator).
- Availability: included on private-client (Legacy / All-Inclusive) plans. On the $97 and Free plans it's an add-on — when it's locked you'll see a short overview and a "Schedule a demo" button. It's a great differentiator to add value to (or upsell into) your wedding packages.
- Free couple wedding website: every couple in Wedding Planner can build a free, mobile-first wedding website inside their own StoryVenue login (couple side → **Website** tab). It's a simple linktree-style page dressed up as a wedding site, hosted at storyvenue.com/their-custom-link — one link to share on invitations, texts, and social. Couples add a photo/cover, names, headline, story, a live countdown, their socials, and up to six custom link buttons (registry, hotel block, travel, livestream, schedule), plus online RSVP (guests find their invite by name and reply with headcount + meal + dietary notes) and a guestbook (couple can approve posts). When the couple turns on the "Our Venue" card, their website links back to your public listing — extra exposure to every guest. They pick a custom link (availability-checked), toggle sections, hit Publish, and can copy the link or download a QR code. To point a couple to it: once connected, tell them to log in and open the Website tab. RSVPs they collect flow into the same guest-list rollup you already see. Great tour/package talking point: "book with us and get a free wedding website." No separate Knot/Zola account needed.
...
- Pipeline (card below header): Use the **Pipeline** dropdown to pick one of your venue's sales pipelines (same pipelines as Leads / Kanban). **Stage** pills list that pipeline's stages with colors from your setup — click a pill to move the contact; saves to the server (UI updates immediately). If a lead exists with the same email, you may see "Linked to lead — stage syncs both ways". Default template stages often include Lead, Conversations Started, Qualified, Tour Booked, Proposal Sent, Wedding Booked, Follow up, Not Interested — venues can rename, add, or reorder stages on the Leads page.
...
- What flows over: first/last name, email, phone, wedding date, guest count, message, and UTM attribution. Every lead is labeled with the source "StoryVenue - Bride Booking System™" so it's easy to track in Tripleseat reporting.
- Use "Send test lead" to confirm it's working. Data only flows from StoryVenue to Tripleseat (one-way). Disconnect anytime.

### Event Temple (venue CRM)
...
- **5 phases** — each has its own toggle so you enable only what you need. Only Phase 1 (Guide Delivery) and Phase 2 (14-Day Sequence) are ON by default; Phases 3 and 4 start OFF so the venue can review/customize the pre-loaded copy first.
  ...
  3. **Booked Tour → Toured** — fires when a lead is moved to the "Tour Booked" pipeline stage. 3 pre-loaded touches (SMS immediate, email immediate, email +2 days) with visit details. Supports {{appointment_date}}, {{appointment_time}}, {{venue_address}} merge variables sourced from her calendar event. No AI Concierge handoff.
  4. **Booked Wedding → Welcomed** — fires when a lead is moved to the "Wedding Booked" pipeline stage. 5 pre-loaded touches celebrating the booking and setting next steps. No AI Concierge handoff.
  5. **AI Concierge** — plan-gated (All-Inclusive only). An AI SMS follow-up for quiet leads. Locked on Bride Booking System Free and Bride Booking System™ plans; greyed toggle with tooltip and demo scheduling modal. See AI Concierge section for full details.
- Note: the previous "Nurture Sequence" phase (5-email educational content about picking/touring venues) was removed from this page — venues who want that kind of standalone educational content can build it as its own Email Campaign instead, keeping Speed to Lead focused on the core inquiry→tour→wedding funnel.
