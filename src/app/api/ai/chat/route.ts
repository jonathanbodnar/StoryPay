...
- AI Concierge is a venue-level SMS-based lead engagement system powered by DeepSeek. When enabled, it automatically contacts new leads via personalized text messages over a 14-day sequence, handles inbound replies, and escalates to humans when needed.
...
- **Activation**: toggle the master switch on the AI Concierge settings page. When activated, a cron job scans for eligible leads and starts the sequence. Leads in "dormant" state with no prior activation are picked up automatically.
- **Lead states**: dormant → ai_active → paused → exhausted → handoff → opted_out. The AI sends messages only when a lead is in "ai_active" state. State transitions are logged in an audit trail.
- **14-day sequence**: the AI sends personalized outreach messages on a schedule (configurable). Messages reference the venue's packages, pricing, and availability. Each message is generated by DeepSeek using a configurable prompt template with merge variables (contact name, venue name, wedding date, etc.).
- **Inbound replies**: when a lead replies to an SMS, the system classifies the intent (question, objection, booking request, opt-out) and either responds with AI or flags for human follow-up. Any inbound reply moves the lead to "Conversation Started" stage in the pipeline and notifies the venue owner + concierge notification emails.
- **Reply-trigger outreach**: leads who submit forms or trigger links but haven't been contacted can also be picked up by the AI Concierge for initial outreach.
...
- {{venue.pricing_guide_url}} — direct download link for the venue's Pricing & Availability Guide PDF; always serves the most up-to-date version. Use this in booking system email and SMS templates so brides get a clickable, downloadable PDF link. Alias: {{pricing_guide_url}}
...
- Two phases covered by the Booking System page:
  ...
  2. 14-Day Follow-up Sequence (Phase 2): A configurable sequence of steps (Send Email, Send SMS, Wait) that runs on a cron in the background. Each Wait step can be set to 1, 2, or 3 days only. Steps can be drag-and-drop reordered.
- Activate AI Concierge block: the sequence can include an "Activate AI Concierge" trigger block. When the cron reaches this block, it immediately activates the AI Concierge for that lead (setting ai_state to ai_active) and marks the enrollment complete. Leads activated this way are excluded from the 14-day cron timer so they don't get double-activated.
...
- Where to configure: Venue listing → Booking System. Guide Delivery settings (email on/off, SMS on/off, email body, SMS body) are at the top; the 14-Day Sequence builder is below.
