...
- Contact info (email, phone, address) shown on documents — used by the marketing email **Address block** as well.
- **Brand Colors palette** (Settings → Branding → Brand Colors): per-venue saved color list (max 50 hex colors) that appears at the bottom of EVERY color picker across the app — email builder (text, buttons, dividers, social icons, backgrounds), proposals, lead capture forms. Click the bookmark/save icon next to any color in any picker to add it to your palette instantly. DB column: venues.brand_colors (jsonb array of hex strings).
- **Social Networks** (Settings → Branding → Social Networks, anchored at #social-networks): per-venue social profile URLs used by the marketing email **Social block**. Supported platforms: Instagram, Facebook, TikTok, LinkedIn, YouTube, Twitter / X, Threads, Pinterest, Website. One URL per platform; up to 12 total. Auto-prefixes https:// when missing; auto-saves with debounce. Each row has Open link + Remove. Single source of truth — Social blocks pull from this list and have no per-email override. DB column: venues.brand_socials (jsonb array of {platform, url} objects, migration 059).
...
- Brand socials → venues.brand_socials (jsonb array of platform+url objects, migration 059). Loaded by the useBrandSocials hook. PATCH /api/venues/me sanitizes input (known platforms only, valid URLs, https:// auto-prefix, max 12, no duplicates).
...
- At render time the helper injectVenueDataIntoDefinition (src/lib/marketing-email-injection.ts) copies venues.brand_socials onto every Social block's socialLinks field — preview, test sends, and bulk worker all go through this so social links are always live.
...
- How do I add my Instagram / TikTok / etc. to my marketing emails? Settings → Branding → Social Networks. Add your URLs there once (Instagram, Facebook, TikTok, LinkedIn, YouTube, Twitter / X, Threads, Pinterest, Website). Every Social block in every campaign automatically uses these. The Email builder Social inspector → Links tab → "Manage in branding" deep-links you straight there.
...
- How do I save a brand color so I can reuse it everywhere? Settings → Branding → Brand Colors → type a hex code → Save. Or, from any color picker across the app, click the bookmark/save icon next to the color — it lands in your palette and shows up at the bottom of every other color picker (email blocks, lead capture forms, branding presets, proposals).
...
- Where are the social links stored technically? venues.brand_socials (jsonb array of {platform, url} — migration 059). Read by the useBrandSocials hook. PATCH /api/venues/me validates platform, requires a valid URL, auto-prefixes https://, dedupes, and enforces a max of 12.
