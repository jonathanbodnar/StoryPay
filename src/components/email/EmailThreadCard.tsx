...
/**
 * Single source of truth for how an email renders inside ANY chat feed —
 * the Venue Concierge thread (venue side + super-admin) and every tab of the
 * super-admin Support Inbox (bride replies, Venue Direct, support tickets).
 *
 * It's a full-width, Gmail-style accordion card. Collapsed it shows the sender,
 * an "Email" tag, the time and a one-line preview. Expanded it reveals the
 * reply, the signature, and the quoted email thread — bold labels, hyperlinked
 * words and shortened URLs are handled by EmailRich / tidyEmailText.
 */
...
export function EmailThreadCard({
  ...
  body: string;
  /** Sender / direction label, e.g. "You → Concierge", "Support — Francine", "Bride". */
  who: string;
  time: string;
  highlighted?: boolean;
  /** Start expanded (used for the latest message in a thread). */
  defaultOpen?: boolean;
  /** Optional node rendered after the time (e.g. a delivery-status icon). */
  trailing?: ReactNode;
}) {
  const { reply, signature, quoted } = useMemo(() => parseEmailParts(body), [body]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="min-w-0 truncate text-[11px] font-semibold uppercase tracking-wide text-gray-500">{who}</span>
