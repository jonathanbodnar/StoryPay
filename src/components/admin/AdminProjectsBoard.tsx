...
import {
  ...
  Eye, Plus, Search, Trash2, GripVertical, Settings2, ListChecks,
} from 'lucide-react';
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4">
              ...
              <div className="flex flex-col gap-2">
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none focus:ring-1 focus:ring-gray-900"
                />
                <button
                  onClick={addNote}
              ...
              <div className="mt-4 space-y-2.5">
                {loadingNotes ? (
                  ...
                  notes.map((n) => (
                    <div key={n.id} className="rounded-lg border border-gray-200 bg-gray-50/60 p-3">
...
function ChecklistCard({ venueId }: { venueId: string }) {
  ...
  const toggle = useCallback(async (key: string, done: boolean) => {
    setState((prev) => ({ ...prev, [key]: done })); // optimistic
    setSavingKey(key);
    try {
      const res = await fetch('/api/admin/projects/checklist', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ venueId, key, done }),
      });
      const json = await res.json();
      if (res.ok && json.checklist) setState(json.checklist);
      else setState((prev) => ({ ...prev, [key]: !done })); // revert on failure
    } catch {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-gray-800">Onboarding checklist</h3>
        <span className="ml-auto text-[11px] font-medium text-gray-400">{doneCount}/{total}</span>
      </div>
      <div className="mb-3 h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
        <div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${pct}%` }} />
      </div>
      {loading ? (
        <div className="flex items-center gap-2 py-6 text-xs text-gray-400"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
      ) : (
